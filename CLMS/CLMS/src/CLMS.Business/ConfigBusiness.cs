...
namespace CLMS.Business
{
    ...
    public class ConfigBusiness
    {
        ...
        public void LicenceConfigEdit(string sysCode, string licenceNumber)
        {
            Dictionary<string, string> updates = new Dictionary<string, string>();
            updates.Add("SysCode", sysCode);
            updates.Add("LicenceNumber", licenceNumber);
            _sysprovider.Update(updates);
            LicenceBusiness.GetInstance().TestLicenceCode();
        }
    }
}
